...
function StatsPage() {
  ...
  const exportCsv = () => {
    ...
    a.href = url;
    a.download = `stats-${tab.toLowerCase()}-${range}.csv`;
    a.click();
          ...
          <div className="flex flex-wrap gap-1 rounded-full border border-border bg-card p-1">
            {RANGES.map((r) => (
              <button
                ...
                className={`rounded-full px-3 py-1 text-xs font-medium ${range === r.key ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}
              >
                {r.label}
              </button>
            ))}
          ...
          <div className="flex flex-wrap gap-1 rounded-full border border-border bg-muted/40 p-1">
            {BREAKDOWNS.map((b) => (
              <button
                key={b}
                onClick={() => setTab(b)}
                className={`rounded-full px-3 py-1 text-xs font-medium ${tab === b ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}
              >
                By {b}
              </button>
            ))}
