...
function AdCodePage() {
  ...
  const code = useMemo(
    () =>
      activeId
        ? buildAdCode({ zoneId: activeId, async: asyncLoad, responsive, customStyle })
        : "// Select a zone to generate ad code.",
          ...
          <div className="mt-4 flex flex-col gap-4">
              ...
              >
                ...
                {list.map((z) => (
                  <option key={z.id} value={z.id}>
                    {z.name} · {z.size} · {z.siteName ?? z.siteId}
                  </option>
                ))}
              </select>
            </FormField>
            <label className="flex items-center justify-between rounded-lg border border-border bg-card px-4 py-3 text-sm">
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  Non-blocking script tag; recommended.
                </div>
              </div>
              <input
                type="checkbox"
                checked={asyncLoad}
                onChange={(e) => setAsyncLoad(e.target.checked)}
            ...
            <label className="flex items-center justify-between rounded-lg border border-border bg-card px-4 py-3 text-sm">
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  Fills available width up to the zone size.
                </div>
              </div>
              <input
                type="checkbox"
          ...
          <SurfaceCard>
            <div className="flex items-start justify-between gap-3">
              ...
              <button
                onClick={copy}
                disabled={!activeId}
                className="inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1.5 text-xs font-medium hover:bg-muted/50 disabled:opacity-50"
              >
            ...
            <div className="mt-4 grid place-items-center rounded-lg border border-dashed border-border bg-muted/40 p-6">
              {selected ? (
                ...
                <div className="text-sm text-muted-foreground">
                  Select a zone to see the preview.
                </div>
              )}
            </div>
          </SurfaceCard>
