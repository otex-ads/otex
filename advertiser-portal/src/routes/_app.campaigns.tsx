  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <Link to="/campaigns/new" className="inline-flex items-center gap-2 rounded-full bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:brightness-110">
      ...
      {campaigns.length === 0 ? (
        <EmptyState
          ...
          action={<Link to="/campaigns/new" className="inline-flex items-center gap-2 rounded-full bg-primary px-4 py-2 text-sm font-medium text-primary-foreground"><Plus className="size-4" /> Create campaign</Link>}
        />
      ) : (
                  ...
                  return (
                    <tr key={c.id} className="border-b border-border last:border-0 hover:bg-muted/20">
                      ...
                      <td className="px-6 py-4">
                        ...
                        <div className="num text-xs text-muted-foreground">{KES(c.bid)} / {c.pricingModel === "CPM" ? "1K impr" : c.pricingModel === "CPC" ? "click" : "conv"}</div>
                      </td>
                      <td className="num px-6 py-4 text-right text-muted-foreground">
                        {KES(c.dailyBudget)} <span className="text-muted-foreground/60">/</span> {KES(c.totalBudget)}
                      </td>
                        ...
                        <div className="flex justify-end gap-1">
                          {c.status === "active" ? (
                            <button
                              ...
                              title="Pause"
                            ><Pause className="size-3.5" /></button>
                          ) : (
                            <button
                          ...
                          <button
                            onClick={() => { if (confirm(`Delete "${c.name}"?`)) store.removeCampaign(c.id); }}
