...
function SiteDetail() {
  ...
  const zones = useQuery({
    ...
    queryFn: () => api.listZones(siteId),
    retry: false,
  });
  const stats = useQuery({
    queryKey: ["stats", "site", siteId],
    queryFn: () =>
      api.stats({ from: isoDaysAgo(30), to: isoDaysAgo(0), groupBy: `site:${siteId}` }),
    retry: false,
  });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <div className="mt-2 flex items-center gap-3 text-sm text-muted-foreground">
            ...
            {site && (
              <StatusPill
                status={site.status[0].toUpperCase() + site.status.slice(1)}
      ...
      <div className="grid gap-4 md:grid-cols-4">
        {[
          {
            label: "Impressions",
            value: Num(stats.data?.summary.impressions ?? site?.impressions ?? 0),
          },
