...
const menu = [
  ...
  { to: "/adcode", label: "Ad Code", icon: Code2 },
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-20 flex items-center justify-between gap-4 border-b border-border bg-background/80 px-4 py-4 backdrop-blur md:px-8 md:py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-base font-bold text-foreground">Publisher Console</div>
              <div className="text-xs text-muted-foreground">
                {user?.email ? `Signed in as ${user.email}` : "Manage sites, zones and earnings"}
              </div>
            </div>
          </div>
